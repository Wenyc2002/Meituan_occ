...
def world_to_pixel(points_xyz, x_range, y_range, cell_size, h, w):
    ...
    # y轴映射到图像x轴（水平方向），实现左右镜像
    px = ((points_xy[:, 1] - y_range[0]) / cell_size).astype(np.int32)  # 镜像：y_range[0]到右，y_range[1]到左
...
class MapGenerator:
    ...
    def pc_callback(self, pc1, pc2, pc3):
        ...
        try:
            ...
            # 设置地图原点（对应livox_frame的(0,0,0)，考虑镜像）
            grid_msg.info.origin = Pose()
            grid_msg.info.origin.position.x = self.map_range_x[0]
            grid_msg.info.origin.position.y = self.map_range_y[0]
            grid_msg.info.origin.position.z = 0.0
            grid_msg.info.origin.orientation.w = 1.0
            
            # 转换map2d为OccupancyGrid数据（0=free, 100=occupied, -1=unknown）
